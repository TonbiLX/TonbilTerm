...
void LocalFallback::loop(bool mqttConnected, bool hasSensor) {
    ...
    switch (_level) {
        case FB_NORMAL:
            // Check if we've exceeded fallback timeout
            if (sinceLast >= _config.fallbackTimeout) {
                if (hasSensor) {
                    enterLocal();
                } else {
                    // No sensor available — go directly to fixed cycle
                    enterFixedCycle();
                }
            }
            break;
        ...
        case FB_LOCAL:
            // Check if we've been in LOCAL for too long (1 hour total without server)
            if (sinceLast >= 3600) {
                enterFixedCycle();
            }
            break;
        ...
        case FB_FIXED_CYCLE:
            // Stay in fixed cycle until server reconnects
...
void LocalFallback::enterFixedCycle() {
    ...
    _cyclePhaseOn = true;  // Start with ON phase
