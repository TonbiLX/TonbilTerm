...
void RelayController::evaluate(float currentTemp) {
    ...
    if (_freezeActive) return;  // Freeze handler controls relay
...
// Returns true if relay state was actually changed
// ============================================================
...
void RelayController::checkFreezeProtection(float temp) {
    if (temp < _config.freezeThreshold) {
        ...
        // Force relay ON — bypasses min cycle and max runtime
        if (!_relayOn) {
            _relayOn = true;
        ...
        // Exit freeze protection with 2C hysteresis to prevent oscillation
        _freezeActive = false;
...
bool RelayController::isCycleLocked() const {
    ...
    if (_relayOn) {
        return ((now - _relayOnSince) / 1000) < _config.minCycleTime;
    } else {
        return ((now - _relayOffSince) / 1000) < _config.minCycleTime;
    }
}
