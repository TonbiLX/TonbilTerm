...
bool BME280Reader::begin(uint8_t addr) {
    ...
    if (_bme.begin(addr, &Wire)) {
        ...
        // Indoor navigation filter -- best for thermostat use
        _bme.setSampling(
...
// ============================================================
// read -- get current sensor values with calibration applied
// ============================================================
BME280Data BME280Reader::read() {
    BME280Data data;

    if (!_available) {
        return data;  // All NaN, valid=false
    }

    float t = _bme.readTemperature();
    float h = _bme.readHumidity();
    float p = _bme.readPressure() / 100.0f;  // hPa
    ...
    data.temperature = t + _calOffsetTemp;
    data.humidity    = h + _calOffsetHum;
    data.pressure    = p;
    data.valid       = true;
